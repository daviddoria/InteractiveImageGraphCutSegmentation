...
void InnerWidget<TImage>::StopProgressSlot()
{
  ...
  vtkSmartPointer<vtkImageData> VTKSegmentMask =
    vtkSmartPointer<vtkImageData>::New();
  Helpers::ITKImagetoVTKImage<MaskImageType>(this->GraphCut.GetSegmentMask(), VTKSegmentMask);

  // Convert the image into a VTK image for display
  vtkSmartPointer<vtkImageData> VTKImage =
    vtkSmartPointer<vtkImageData>::New();
  Helpers::ITKImagetoVTKImage<TImage>(this->GraphCut.GetMaskedOutput(), VTKImage);
...
template <typename TImage>
float InnerWidget<TImage>::ComputeLambda()
{
...
template <typename TImage>
void InnerWidget<TImage>::UpdateLambda()
{
...
template <typename TImage>
void InnerWidget<TImage>::sldHistogramBins_valueChanged()
{
  this->GraphCut.SetNumberOfHistogramBins(sldHistogramBins->value());
}

template <typename TImage>
void InnerWidget<TImage>::radForeground_clicked()
{
  this->GraphCutStyle->SetInteractionModeToForeground();
}

template <typename TImage>
void InnerWidget<TImage>::radBackground_clicked()
{
  this->GraphCutStyle->SetInteractionModeToBackground();
}

template <typename TImage>
void InnerWidget<TImage>::btnClearSelections_clicked()
{
  this->GraphCutStyle->ClearSelections();
}

template <typename TImage>
void InnerWidget<TImage>::btnSaveSelections_clicked()
{
...
template <typename TImage>
void InnerWidget<TImage>::btnCut_clicked()
{
...
#if 0
template <typename TImage>
void InnerWidget<TImage>::actionSave_Segmentation_triggered()
{
